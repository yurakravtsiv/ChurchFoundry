...
Перед кожним комітом Husky запускає:

1. **lint-staged** — Biome лише на staged `.ts`/`.tsx` файлах (з автофіксом)
2. **vitest run** — усі тести один раз

Якщо лінтер або тести падають, коміт блокується. У крайньому випадку можна обійти хук через `git commit --no-verify`, але цим краще не зловживати — перевірки потрібні, щоб у `main` не потрапляв зламаний код.
