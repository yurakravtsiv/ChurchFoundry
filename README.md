...
- **pnpm**
- **Supabase** — Auth уже використовується; дані інвентарю та профіль церкви поки в **localStorage**, з готовою архітектурою переходу на Supabase через TanStack Query хуки
...
- **Інвентар** (повний модуль):
  - CRUD предметів
  - категорії / підкатегорії / локації / відповідальні зі створенням «на льоту» та CRUD на сторінці **Налаштування** (вертикальні таби)
  - фільтри та пошук по видимих колонках
  - налаштування колонок таблиці (видимість, порядок drag-and-drop; той самий вигляд у пошуку та експорті)
  - QR-коди для кожного предмета (логотип церкви в центрі, якщо заданий)
  - списання / повернення в наявність з історією
  - експорт у XLSX / PDF
  - вибір дат через локалізований DatePicker (місяць/рік)
- **Налаштування** — сторінка з вертикальними табами: профіль **церкви** (назва, адреса з пошуком, телефон, email, сайт, логотип), довідники інвентаря (категорії / підкатегорії / локації / відповідальні) і генерація тестових даних
- **Брендинг церкви** — логотип церкви в центрі QR інвентарю; неавторизований скан відкриває картку з контактами церкви і можливістю увійти
...
- **localStorage як тимчасовий шар даних**, обгорнутий у TanStack Query хуки — перехід на реальний backend (Supabase) вимагатиме зміни переважно `queryFn` / `mutationFn`; UI-шар (loading / error / invalidate) лишається тим самим. Профіль церкви зараз теж у localStorage, тож на іншому пристрої картка після скану QR з’явиться після перенесення в БД.
